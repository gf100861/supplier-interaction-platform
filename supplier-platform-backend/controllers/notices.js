...
const LIST_NOTICE_SELECT = `
    ...
    status,
    created_at,
    sd_description:sd_notice->description,
    sd_details:sd_notice->details,
    sd_create_time:sd_notice->createTime,
    creator:users ( id, username, email, role ),
    supplier:suppliers ( parma_id, short_code )
`;
...
function withListPreviewFields(row) {
    if (!row) return row;
    const {
        sd_description: sdDescription,
        sd_details: sdDetails,
        sd_create_time: sdCreateTime,
        ...listRow
    } = row;
    ...
    return {
        ...listRow,
        sd_notice: {
            createTime: sdCreateTime || row.created_at,
