...
import { Layout, Menu, Typography, Card, Row, Col, Anchor, Divider, Space, Tag, Button, Breadcrumb, Steps, Input, AutoComplete, Collapse, Timeline } from 'antd';
import { 
    ...
    <div id="quick-start" className="animate-fade-in">
        ...
        <Card bordered={false} className="steps-card">
            <Steps current={-1} progressDot items={[
                { title: '登录系统', description: '使用分配的账户登录' },
                { title: '查看仪表盘', description: '了解待办事项和KPI' },
                { title: '处理通知单', description: '提交计划或审核证据' },
                { title: '查看报告', description: (<span><b>导出综合报告</b></span>) },
            ]} />
        </Card>
    </div>
);
            ...
            <Row gutter={24} align="middle">
                ...
                <Col xs={24} md={10}>
                    <div className="ai-demo-box">
                        <div className="bubble user">查询防锈油问题...</div>
                        <div className="bubble ai">根据历史数据，共发现3起相关问题，主要集中在...</div>
                    </div>
                </Col>
            </Row>
        </div>
            ...
            <div style={{ marginTop: 24 }}>
                <Collapse accordion defaultActiveKey={['0']} size="large" bordered={false} style={{ background: 'transparent' }}>
                    {faqs.map((faq, idx) => (
                        ...
                        >
                            <p style={{ color: '#666', paddingLeft: 24 }}>{faq.a}</p>
                        </Collapse.Panel>
                    ))}
                </Collapse>
            </div>
