...
import { Layout, Menu, Input, Button, List, Card, Typography, Spin, Avatar, Space, Tooltip, Rate, Modal, Popconfirm, message } from 'antd'; // 1. 引入 message
...
const { TextArea } = Input; // TextArea 在这里未被使用，但保留以防万一
...
const IntelligentSearchPage = () => {
    ...
    const fetchSessions = async () => {
        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                handleNewChat(false); // 自动创建新会话（但不激活）
            }
        } catch (error) {
    ...
    // --- (其他函数: handleDeleteSession, handleFeedback, handleRatingSubmit, showDetailsModal... 保持不变) ---
    // ...
    ...
    const handleRatingSubmit = async () => {
         if (currentRating === 0 && !ratingComment.trim()) {
             message.warning('请至少选择星级或填写评论后再提交。');
             return;
         }
         if (!currentUser?.id) {
         ...
         const ratingData = {
             ...
             session_id: activeSessionId // 关联到当前会话
         };
         ...
         try {
             ...
             console.log(`User rated session: ${currentRating} stars, Comment: ${ratingComment}`);
             ...
             messageApi.error(`提交评价失败: ${error.message}`);
         }
     };
    ...
    return (
        <Layout className="intelligent-search-layout" style={{ height: 'calc(100vh - 140px)', // 调整以适应父布局
         maxWidth: 1200, margin: '0 auto', border: '1px solid #f0f0f0', borderRadius: '8px' }}>
