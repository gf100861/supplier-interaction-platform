...
// 引入 Grid 用于响应式断点检测
import { Form, Input, Button, Upload, Typography, Divider, Modal, Select, InputNumber, Card, Row, Col, DatePicker, Grid, Checkbox } from 'antd';
import { UploadOutlined, InboxOutlined, VideoCameraOutlined } from '@ant-design/icons';
import { useSuppliers } from '../contexts/SupplierContext';
import { useCategories } from '../contexts/CategoryContext';
import { useNotification } from '../contexts/NotificationContext';
import { useNotices } from '../contexts/NoticeContext';
import { uploadFileToCos } from '../services/cosUpload';
...
const FileUploadPage = () => {
  ...
  const onFinish = async (values) => {
    ...
    try {
          ...
          fileList.map(async (file) => {
            if (file.originFileObj) {
              try {
                ...
                const uploaded = await uploadFileToCos(uploadFile, uploadName);
                return {
                  uid: file.uid,
                  name: uploadName,
      ...
      messageApi.open({ type: 'loading', content: '正在保存整改通知单...', key: 'submitting', duration: 0 });
      ...
      const newNoticeToInsert = {
        ...
        sd_notice: {
          ...
          requireSupplierEvidenceFiles: !!values.requireSupplierEvidenceFiles,
        },
        history: [],
      };
  ...
  const beforeUpload = (file) => {
    ...
    // 限制 50MB (视频容易超大)
    const maxSizeMb = file.type.startsWith('video/') ? 300 : 50;
    const isWithinLimit = file.size / 1024 / 1024 < maxSizeMb;
    if (!isWithinLimit) {
      messageApi.error(`文件必须小于 ${maxSizeMb}MB!`);
      return Upload.LIST_IGNORE;
    }
    return false; // 阻止自动上传，手动控制
  };

  const handleCapturedVideo = (event) => {
    const selectedFiles = Array.from(event.target.files || []);
    const validFiles = selectedFiles.filter(file => beforeUpload(file) === false);

    if (validFiles.length > 0) {
      const currentFiles = form.getFieldValue('images') || [];
      const capturedFiles = validFiles.map((file, index) => ({
        uid: `captured-video-${Date.now()}-${index}`,
        name: file.name || `recording-${Date.now()}.mp4`,
        type: file.type || 'video/mp4',
        size: file.size,
        originFileObj: file,
      }));

      form.setFieldValue('images', [...currentFiles, ...capturedFiles]);
    }

    // Allow recording the same file name again after deleting/retrying.
    event.target.value = '';
  };
    ...
    <div style={{ maxWidth: 800, margin: 'auto', padding: isMobile ? '12px' : '24px 0' }}>
      <Card bodyStyle={{ padding: isMobile ? '12px' : '24px' }}>
        ...
        <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ date: dayjs() }}>
          ...
          <Form.Item
            name="requireSupplierEvidenceFiles"
            valuePropName="checked"
            initialValue={false}
          >
            <Checkbox>要求供应商提交完成证据时必须上传图片/视频或附件</Checkbox>
          </Form.Item>
          ...
          <Form.Item label="证据上传 (图片/视频)">
            ...
            <Form.Item name="images" valuePropName="fileList" getValueFromEvent={(e) => Array.isArray(e) ? e : e?.fileList} noStyle>
              ...
              >
                ...
                <p className="ant-upload-text">点击选择图片或视频</p>
                <p className="ant-upload-hint">图片最大 50MB，视频最大 300MB</p>
              </Dragger>
            </Form.Item>
          </Form.Item>
