...
const NoticePage = () => {
    ...
    const handlePlanSubmit = async (values) => {
        const notice = await ensureNoticeDetail(selectedNotice);
        if (!notice) return;
    ...
    const ensureNoticeDetail = async (notice) => {
        if (!notice?.id) return notice;
        const hasHistory = Array.isArray(notice.history) && notice.history.length > 0;
        if (!notice.isLightweight && hasHistory) return notice;

        try {
            return await fetchNoticeDetail(notice.id);
        } catch (error) {
            console.error('Fetch notice detail before action failed:', error);
            messageApi.error('读取通知单详情失败，请刷新后重试');
            return null;
        }
    };
    ...
    const handlePlanApprove = async (targetNotice) => {
        ...
        const notice = await ensureNoticeDetail(targetNotice || selectedNotice);
        if (!notice) return;
    ...
    const handlePlanReject = async (values, noticeArg) => {
        const notice = await ensureNoticeDetail(noticeArg || rejectionModal.notice);
        if (!notice) return;
    ...
    const handleEvidenceSubmit = async (values) => {
        ...
        try {
            ...
            const requireSupplierEvidenceFiles = !!notice?.sdNotice?.requireSupplierEvidenceFiles;
            const missingRequiredFileIndex = requireSupplierEvidenceFiles
                ? originalPlans.findIndex((_, index) => {
                    const evidenceItem = values.evidence?.[index] || {};
                    const hasImages = Array.isArray(evidenceItem.images) && evidenceItem.images.length > 0;
                    const hasAttachments = Array.isArray(evidenceItem.attachments) && evidenceItem.attachments.length > 0;
                    return !hasImages && !hasAttachments;
                })
                : -1;

            if (missingRequiredFileIndex >= 0) {
                messageApi.error({
                    content: `SD 已要求提交佐证文件，第 ${missingRequiredFileIndex + 1} 项请至少上传一个图片/视频或附件。`,
                    key: 'evidenceSubmit',
                    duration: 4
                });
                return;
