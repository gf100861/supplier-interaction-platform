...
import { signCosFileList } from '../../services/cosUpload';
...
const getMediaUrl = (media) => {
    ...
    return media.url || media.thumbUrl || media.preview || '';
};

const getMediaName = (media, index) => {
    if (!media || typeof media === 'string') {
        const url = typeof media === 'string' ? media : '';
        return decodeURIComponent(url.split('/').pop()?.split('?')[0] || `media-${index + 1}`);
    }
    return media.name || `media-${index + 1}`;
};

const isVideoMedia = (media, index) => {
    const type = typeof media === 'object' ? (media.type || '').toLowerCase() : '';
    const name = getMediaName(media, index).toLowerCase();
...
const isUploadImage = (file) => {
    const type = file?.type || file?.originFileObj?.type || '';
    const name = file?.name || file?.url || file?.thumbUrl || '';
    return type.startsWith('image/') || /\.(png|jpe?g|gif|webp|bmp|svg)$/i.test(name.split('?')[0]);
};
...
const LocalImageCarousel = ({ images, title }) => {
    ...
    const hasVideo = images.some((item, index) => isVideoMedia(item, index));
            ...
            <div style={{ padding: '0 20px' }}> {/* 增加 padding 以防止箭头遮挡 */}
                ...
                >
                    {images.map((img, index) => {
                        ...
                        const isVideo = isVideoMedia(img, index);
                        return (
                            <div key={index}>
                                <div style={contentStyle}>
                                    {isVideo ? (
                                        <video
                                            ...
                                            controls
                                            preload="metadata"
                                            style={{ maxHeight: '100%', maxWidth: '100%', objectFit: 'contain', background: '#000' }}
                                        >
...
const PlanSubmissionForm = ({ onFinish, form, actionAreaStyle, notice }) => {
    ...
    return (
        <div style={actionAreaStyle}>
            ...
            {notice?.sdNotice?.requireSupplierEvidenceFiles ? (
                <Alert
                    type="warning"
                    showIcon
                    style={{ marginBottom: 16 }}
                    message="SD 已要求供应商提交佐证文件，每个行动项至少需要上传一个图片/视频或附件。"
                />
            ) : (
                <Alert
                    type="info"
                    showIcon
                    style={{ marginBottom: 16 }}
                    message="SD 未强制要求上传佐证文件，图片/视频和附件为可选。"
                />
            )}
...
export const NoticeDetailModal = ({
    ...
    const [signedInitialImages, setSignedInitialImages] = useState([]);
    const [signedInitialAttachments, setSignedInitialAttachments] = useState([]);
    const [signedEvidencePlans, setSignedEvidencePlans] = useState([]);
    ...
    useEffect(() => {
        ...
        const signInitialFiles = async () => {
            const images = notice?.sdNotice?.images || [];
            const attachments = notice?.sdNotice?.attachments || [];
            const history = notice?.history || [];
            const lastEvidence = [...history].reverse().find(h => h.type === 'supplier_evidence_submission');
            const evidencePlans = lastEvidence?.actionPlans || [];

            try {
                const [nextImages, nextAttachments, nextEvidencePlans] = await Promise.all([
                    signCosFileList(images),
                    signCosFileList(attachments),
                    Promise.all(evidencePlans.map(async (plan) => ({
                        ...plan,
                        evidenceImages: await signCosFileList(plan.evidenceImages || []),
                        evidenceAttachments: await signCosFileList(plan.evidenceAttachments || plan.attachments || []),
                    }))),
                ]);

                if (!cancelled) {
                    setSignedInitialImages(nextImages);
                    setSignedInitialAttachments(nextAttachments);
                    setSignedEvidencePlans(nextEvidencePlans);
                }
            } catch (error) {
                console.warn('Failed to sign COS files for notice detail:', error);
                if (!cancelled) {
                    setSignedInitialImages(images);
                    setSignedInitialAttachments(attachments);
                    setSignedEvidencePlans(evidencePlans);
                }
            }
        };
        ...
        if (open && notice?.id) {
            ...
            setSignedInitialImages([]);
            setSignedInitialAttachments([]);
            setSignedEvidencePlans([]);
        }

        return () => {
            cancelled = true;
        };
    }, [open, notice?.id, notice?.sdNotice?.images, notice?.sdNotice?.attachments, notice?.history]);
    ...
    const handlePreview = async (file) => {
        const type = file.type || file.originFileObj?.type || (file.name?.match(/\.(mp4|webm|ogg|mov|m4v)$/i) ? 'video/mp4' : 'image/jpeg');
        if (!file.url && !file.preview && file.originFileObj) {
            file.preview = type.startsWith('video/')
    ...
    const renderActionArea = () => {
        ...
        switch (notice.status) {
            ...
            case '待SD关闭evidence': {
                ...
                const evidenceList = signedEvidencePlans.length > 0
                    ? signedEvidencePlans
                    : (lastEvidence?.actionPlans || []);
        ...
        >
            ...
            {notice?.category === 'Historical 8D' ? (
                ...
                <Card size="small" type="inner">
                    ...
                    <LocalImageCarousel images={signedInitialImages} title="初始图片" />
                    <AttachmentsDisplay attachments={signedInitialAttachments} />
            ...
            <Modal open={previewOpen} title={previewTitle} footer={null} onCancel={handleCancel} width={isMobile ? '100%' : undefined} style={isMobile ? { top: 20 } : {}}>
                {previewType.startsWith('video/') ? (
                    <video src={previewImage} controls style={{ width: '100%', maxHeight: '70vh', background: '#000' }} />
                ) : (
